...
import com.xiaohelab.guard.android.feature.clue.ReportClueUiEffect
import com.xiaohelab.guard.android.feature.clue.ReportClueViewModel

/** PUB-03 登录用户上报线索 */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun ReportClueScreen(
    onNavigateBack: () -> Unit,
    onNavigateToReceipt: (clueId: String) -> Unit,
    viewModel: ReportClueViewModel = hiltViewModel()
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.effect.collect { effect ->
            when (effect) {
                is ReportClueUiEffect.NavigateToReceipt -> onNavigateToReceipt(effect.clueId)
            }
        }
    }
    ...
    Scaffold(
        ...
        Column(
            ...
            OutlinedTextField(
                ...
                label = { Text("目击位置（选填）") }, singleLine = true,
                modifier = Modifier.fillMaxWidth()
            )
            OutlinedTextField(
                value = state.contactPhone, onValueChange = viewModel::onContactPhoneChange,
