...
5. 新增 clue_evidence_request 资源表，承接 3.2.13 管理端补证请求生命周期。
6. 新增 guardian_invitation 资源表，承接监护邀请生命周期；并为 transfer_request_id 增加全局唯一约束。
...
- 相比早期 11+1 基线，本版新增/显式化了 sys_config、consumed_event_log、notification_inbox、guardian_invitation、clue_evidence_request。
- notification_inbox 为 API 3.9 消息中心接口所需持久化能力，不新增业务域，仅补齐治理域通知能力落库。
- guardian_invitation 承接 3.3 监护邀请生命周期（PENDING/ACCEPTED/EXPIRED/REVOKED），sys_user_patient 保持已生效关系快照。
- clue_evidence_request 承接 3.2.13 线索补证流程（发起、提交、超时/撤销）。
...
| transfer_state | varchar(32) | null | NONE/PENDING_CONFIRM/ACCEPTED/REJECTED/CANCELLED/EXPIRED |
...
### 4.5A clue_evidence_request（线索补证请求）

| 字段 | 类型 | 约束 | 说明 |
| :--- | :--- | :--- | :--- |
| id | bigint | PK | 主键 |
| evidence_request_id | varchar(64) | unique not null | 补证请求号 |
| clue_id | bigint | not null | 关联线索 |
| requested_by | bigint | not null | 发起管理员 |
| evidence_type | jsonb | not null | 补证类型数组（PHOTO/VIDEO/GEO_META） |
| reason | varchar(256) | not null | 补证原因 |
| deadline | timestamptz | null | 截止时间 |
| status | varchar(20) | not null default 'PENDING' | PENDING/SUBMITTED/EXPIRED/CANCELLED |
| submitted_at | timestamptz | null | 补证提交时间 |
| evidence_urls | jsonb | null | 补交证据地址数组 |
| created_at | timestamptz | not null | 创建时间 |
| updated_at | timestamptz | not null | 更新时间 |
...
- check(jsonb_typeof(evidence_type)='array')

索引：

- idx_evidence_req_clue(clue_id, created_at desc)
- idx_evidence_req_status_deadline(status, deadline)
- idx_evidence_req_requested_by(requested_by, created_at desc)
...
| clue_evidence_request.clue_id | clue_record.id | 补证请求关联线索 |
| clue_evidence_request.requested_by | sys_user.id | 补证请求发起人 |
...
| evidence_request_status | PENDING/SUBMITTED/EXPIRED/CANCELLED |
...
| config_scope | public/ops/security |
...
| 3.2.13 发起补证请求 | evidence_request_id/evidence_type/reason/deadline/status | clue_evidence_request | evidence_request_id/evidence_type/reason/deadline/status |
...
| 3.8.13 DEAD 重放 | event_id/replay_reason/next_retry_at | sys_outbox_log / sys_log | phase( DEAD->RETRY )、replay_reason、replay_token、last_intervention_* |
...
8. short_code 发号依赖的 `patient_short_code_seq` 必须纳入主备切换 Runbook，切换后先做序列水位校准再恢复建档流量。
9. `sys_outbox_log` 的 DEAD 干预必须保留 replay_reason/replay_token/last_intervention_*，用于复盘与追责。
