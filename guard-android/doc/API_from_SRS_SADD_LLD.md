...
| created_at | string | string | 任务创建时间（ISO-8601） |
...
| assigned_to | int64 | string | 复核责任人 |
| reported_at | string | string | 上报时间（ISO-8601） |
...
| 字段 | 类型 | 必填 | 规则 |
| :--- | :--- | :---: | :--- |
| evidence_type | array<string> | 是 | PHOTO/VIDEO/GEO_META |
| reason | string | 是 | 5-256 |
| deadline | string | 否 | ISO-8601 |

错误码：E_GOV_4011、E_GOV_4030、E_CLUE_4043、E_REQ_4001、E_REQ_4005。

请求示例：
```http
POST /api/v1/admin/clues/2001/request-evidence HTTP/1.1
Authorization: Bearer <access_token>
X-Trace-Id: trc_demo_20260406_001
X-Request-Id: req_demo_20260406_001
Content-Type: application/json
...
```json
{
    ...
    "data":  {
                 "clue_id":  "2001",
                 "evidence_request_id":  "erq_20260406_0001",
                 "evidence_type":  [
                                       "PHOTO",
                                       "GEO_META"
                                   ],
...
| page_no | int32 | 否 | >=1，默认 1 |
| page_size | int32 | 否 | 1-100，默认 20 |
| status | string | 否 | PENDING/ACCEPTED/EXPIRED/REVOKED |
...
| status | string | string | PENDING/ACCEPTED/EXPIRED/REVOKED |
...
| page_no | int32 | 否 | >=1，默认 1 |
| page_size | int32 | 否 | 1-100，默认 20 |
| transfer_state | string | 否 | PENDING_CONFIRM/ACCEPTED/REJECTED/CANCELLED/EXPIRED |
...
| transfer_state | string | string | PENDING_CONFIRM/ACCEPTED/REJECTED/CANCELLED/EXPIRED |
...
| 字段 | 逻辑类型 | JSON 类型 | 说明 |
| :--- | :--- | :--- | :--- |
| tracking_node | string | string | 物流节点 |
| status | string | string | 节点状态 |
| location_detail | string | string | 节点位置描述 |
| event_time | string | string | 节点时间（ISO-8601） |

错误码：E_GOV_4011、E_MAT_4030、E_MAT_4041、E_REQ_4005。

请求示例：
```http
GET /api/v1/material/orders/5001/tracking?page_no=1&page_size=20 HTTP/1.1
Authorization: Bearer <access_token>
X-Trace-Id: trc_demo_20260406_001
```
...
```json
{
    ...
    "data":  {
                 "items":  [
                               {
                                   "tracking_node":  "上海分拨中心",
                                   "status":  "IN_TRANSIT",
                                   "location_detail":  "上海市浦东新区",
                                   "event_time":  "2026-04-06T11:09:00Z"
                               }
                           ],
                 "page_size":  20,
                 "total":  1,
...
| assistant_reply | string | AI 回复文本 |
| token_usage | object | 本次 token 消耗 |
| token_usage.prompt_tokens | number | 输入 token |
| token_usage.completion_tokens | number | 输出 token |
| token_usage.total_tokens | number | 总 token |
| token_usage.model_name | string | 实际使用模型标识（可选） |
| token_usage.billing_source | string | 计费来源（可选） |
| token_usage.provider_request_id | string | 供应商请求追踪 ID（可选） |
| fallback_response | object | 降级模式响应（可空） |
| fallback_response.mode | string | RULE_BASED/SAFE_GUARD/TOOL_DEGRADED |
...
| order_status | string | 否 | 逗号分隔状态；默认 PENDING,PROCESSING；支持 PENDING/PROCESSING/CANCEL_PENDING/SHIPPED/EXCEPTION/COMPLETED/CANCELLED |
...
| order_status | string | string | 工单状态 |
...
错误码：E_GOV_4032、E_USR_4002、E_USR_4041、E_REQ_4001、E_REQ_4005。
...
```json
{
    ...
    "code":  "OK",
    "data":  {
                 "export_job_id":  "exp_20260406_0001",
                 "status":  "PENDING",
                 "file_url":  null,
                 "created_at":  "2026-04-06T11:44:00Z"
             },
    "trace_id":  "trc_demo_20260406_001"
}
```
...
3. 成功重放后写入 `last_intervention_by/last_intervention_at/replay_reason/replay_token`（`replay_token` 映射自 X-Request-Id）。
4. 重放动作必须落审计并保留 trace_id。
...
| EX-016 | POST | /api/v1/admin/clues/{clue_id}/request-evidence | 线索域 | COMMAND | ADMIN/SUPERADMIN | G_WRITE + E_CLUE_4043 | P2 | 已落地 | 3.2.13 |
...
| EX-028 | GET | /api/v1/material/orders/{order_id}/tracking | 物资域 | LIST | FAMILY | G_READ + E_MAT_4041 | P2 | 已落地 | 3.4.24 |
