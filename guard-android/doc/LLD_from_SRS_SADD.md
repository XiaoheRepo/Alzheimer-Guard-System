...
| clue-intake-service | 线索域（入口） | clue_record（原始段） | 匿名线索入口、标准化与入站削峰 | clue.reported.raw | - |
...
| ai-vectorizer-service | AI 域 | vector_store | 文本切片、向量写入、失效清理 | - | profile.updated、profile.corrected、profile.deleted.logical、memory.appended、memory.expired |
...
3. 校验匿名凭据一次性与绑定关系。
4. 写 clue_record(raw) 并发布 clue.reported.raw。
5. 由研判服务异步处理，不在入口同步执行重计算。
...
| 类别 | 表名 |
| :--- | :--- |
| 核心业务表 | sys_user、patient_profile、sys_user_patient、rescue_task、clue_record、clue_evidence_request、patient_trajectory、tag_asset、tag_apply_record、ai_session、vector_store、sys_log、sys_config |
...
| requested_by | bigint | not null | 发起人（管理员） |
| evidence_type | jsonb | not null | 申请补证类型数组（PHOTO/VIDEO/GEO_META） |
| reason | varchar(256) | not null | 补证原因 |
| deadline | timestamptz | null | 补证截止时间 |
| status | varchar(20) | not null default 'PENDING' | PENDING/SUBMITTED/EXPIRED/CANCELLED |
| submitted_at | timestamptz | null | 实际提交时间 |
| evidence_urls | jsonb | null | 补交证据地址列表 |
| created_at | timestamptz | not null | 创建时间 |
| updated_at | timestamptz | not null | 更新时间 |
...
- idx_evidence_req_created_by(requested_by, created_at desc)
...
| operator | varchar(64) | not null | 操作者 |
...
| processed_at | timestamptz | not null | 处理时间 |
| trace_id | varchar(64) | not null | 链路标识 |

唯一约束：
- uq_consumer_event(consumer_name, topic, event_id)
...
核心流转：
- UNBOUND -> ALLOCATED -> BOUND。
- BOUND -> LOST。
- LOST/VOID 仅管理员可 reset/recover。
...
审计字段基线：
- operator、object、action、result、trace_id、risk_level、detail。
