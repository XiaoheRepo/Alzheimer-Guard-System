...
> - **承载服务**：`AdminUserController` 归属 Authentication Service（唯一持有密码哈希与 JWT 吊销黑名单的域），`AdminPatientController` 归属 Profile Service（唯一持有患者聚合根与监护关系）；两个控制器共享 `@AdminRoleGuard` 注解（详见《Backend Handbook V2》§24）。
...
> 以上 `user.*` 与 `patient.primary_guardian.force_transferred` 属于 V2.1 增量治理事件，对应 SRS FR-GOV-011 ~ FR-GOV-014、FR-PRO-011 / FR-PRO-012；详细 payload 字段与消费侧动作见《Backend Handbook V2》§24。
