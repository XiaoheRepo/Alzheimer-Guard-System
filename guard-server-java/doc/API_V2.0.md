...
| `E_MAT_4093` | 409 | 标签库存不足（`UNBOUND` 池中可用数 < `quantity`） |
...
| `E_AUTH_4031` | 403 | 角色或确认等级不足 |
| `E_GOV_4091` | 409 | `username` 已存在（注册唯一性冲突） |
| `E_GOV_4092` | 409 | `email` 已存在（注册唯一性冲突） |
...
| `E_USR_4095` | 409 | `phone` 已存在（注册 / 修改资料时唯一性冲突） |
...
| `E_MAT_4093` | 409 | `APPROVE` 时标签库存不足（建议先走§3.4.8 批量发号补充库存后重试） |
| `E_MAT_4030` | 403 | 非管理员 |
...
```json
{
  ...
  "phone":    "string, 必填, 中国大陆手机号, 正则 ^1[3-9]\\d{9}$",
...
| `E_AUTH_4001` | 400 | `username` 格式非法 |
| `E_AUTH_4002` | 400 | `password` 格式非法 |
| `E_USR_4005` | 400 | `phone` 格式非法（复用字段格式错误码） |
| `E_GOV_4091` | 409 | 用户名已存在 |
| `E_GOV_4092` | 409 | 邮箱已存在 |
| `E_USR_4095` | 409 | 手机号已存在 |

**处理逻辑**：密码 BCrypt 哈希后存储；`phone` 经唯一性校验后落库（HC-07 列上服务端 `@Desensitize(PHONE)`）；发送邮箱验证链接（TTL = 30 分钟）。
...
| 错误码 | HTTP | 触发条件 |
| :--- | :---: | :--- |
| `E_AUTH_4011` | 401 | 凭据错误（用户名不存在 或 密码不匹配；为避免账号枚举，两者返回同一错误） |
| `E_REQ_4291` | 429 | 同一用户名 / 同一 IP 连续登录失败 ≥5 次，服务端制入 60s 冷却；响应 `Retry-After` = 剩余冷却秒数 |
| `E_GOV_4031` | 403 | 账号已被封禁（`status = DISABLED`） |

> **登录失败保护策略**：
> - 不单独返回「账号锁定」错误码（避免被用于 DOS 锁他人账号）；实现上锁定期间统一走 `E_REQ_4291` + `Retry-After`。
> - 客户端在连续遇到 5 次 `E_AUTH_4011` 后需本地锁按钮 60s，文案“请稍后再试”；后续请求服务端返 `E_REQ_4291` 时依赖 `Retry-After` 倒计时。
> - 上述阈值（5 次 / 60s）为平台默认，以 `GET /api/v1/admin/configs` 返回的 `auth.login.fail_threshold` / `auth.login.cooldown_seconds` 为准（HC-05）。
